...
function generatePages(category: WorkCategory, isMobile: boolean) {
  ...
  pages.push({
    id: "toc",
    content: (
      <div className="w-full h-full bg-white p-8 md:p-12 pb-16 md:pb-24 flex flex-col shadow-[inset_2px_0_10px_rgba(107,86,54,0.05)] relative">
  ...
  category.projects.forEach((p, pIndex) => {
    ...
    const chunk1 = images.splice(0, 1);

    pages.push({
      id: `project-${pIndex}-page-1`,
      content: (
        <div className={`w-full h-full bg-paper p-6 md:p-10 pb-16 md:pb-20 flex flex-col relative ${shadowClass1}`}>
          {/* Text Section */}
          <div className="shrink-0 mb-4">
            <span className="eyebrow text-ink/60">{p.tag}</span>
            <h3 className="font-display mt-2 text-[clamp(1.2rem,1.2rem+1vw,2rem)] font-light leading-tight tracking-tight text-ink mb-3">
              {p.title}
            </h3>
            <p className="text-[10px] md:text-sm leading-relaxed text-ink/80 font-sans line-clamp-4">
              {p.description}
            </p>
          </div>
            ...
            <div className="flex-1 flex flex-col gap-4 min-h-0 relative">
              {chunk1.map((img, idx) => (
                <div key={idx} className="flex-1 relative w-full overflow-hidden bg-ink/5 rounded-sm">
                  <Image src={img} fill sizes="(max-width: 768px) 100vw, 50vw" priority={pIndex === 0} className="object-contain p-2" alt={p.title} />
                </div>
              ))}
            </div>
          )}
    ...
    // Subsequent Pages: Up to 2 images per page
        ...
        content: (
          <div className={`w-full h-full bg-paper p-6 md:p-10 pb-16 md:pb-20 flex flex-col relative ${shadowClassN}`}>
            <div className="flex-1 flex flex-col gap-4 min-h-0 relative">
              {chunk.map((img, idx) => (
                <div key={idx} className="flex-1 relative w-full overflow-hidden bg-ink/5 rounded-sm">
                  <Image src={img} fill sizes="(max-width: 768px) 100vw, 50vw" className="object-contain p-2" alt={p.title} />
                </div>
              ))}
            </div>
...
export default function MagazineFlip() {
  ...
  return (
    <div className="w-full bg-paper font-sans relative">
      ...
      <div className={`w-full pt-20 pb-32`}>
        ...
        <div className={`transition-opacity duration-500 text-center mb-16 md:mb-24 px-6 mx-auto max-w-3xl ${activeCategoryIndex !== null ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
          <h1 className="font-display text-[clamp(2.5rem,2rem+3vw,4.5rem)] font-light text-ink mb-4 md:mb-6">Explore our Catalog</h1>
        ...
        <div className="w-full max-w-6xl mx-auto px-6 flex flex-col gap-32 md:gap-40">
          {allBooks.map((book, idx) => {
            ...
            return (
              <div key={book.cat.id} className="relative flex flex-col md:flex-row items-center gap-12 md:gap-20 w-full">
                {/* TEXT SECTION */}
                <div className={`flex-1 w-full transition-opacity duration-500 ${!isGrid ? 'opacity-0 pointer-events-none' : 'opacity-100'} ${reversed ? 'md:order-2 md:pl-10' : 'md:order-1 md:pr-10'}`}>
                  ...
                  <h2 className="font-display text-[clamp(2rem,1.5rem+2.4vw,3.75rem)] font-light leading-tight tracking-tight text-ink mb-6">
                    {book.cat.heading}
                  </h2>
                ...
                {/* BOOK SECTION */}
                <div className={`flex-1 w-full relative ${reversed ? 'md:order-1' : 'md:order-2'}`}>
                  {/* GHOST PLACEHOLDER */}
                  ...
                  <motion.div
                    ...
                    className={isSelected
                      ...
                      : `relative w-full aspect-[1/1.3] max-h-[50vh] md:max-h-[60vh] mx-auto cursor-pointer group py-4 ${returningIndex === idx ? "z-[70]" : "z-10"}`}
                    style={{
